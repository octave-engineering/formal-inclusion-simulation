      ...
      <div className="max-w-7xl mx-auto animate-fade-in">
        ...
        <div className="sticky top-0 z-10 lg:relative lg:z-0 bg-gradient-to-br from-white to-bg-secondary shadow-lg lg:shadow-card-lg border-b lg:border border-border-light transition-all lg:rounded-2xl lg:mx-6 lg:mt-6 p-4 sm:p-6 lg:p-8 backdrop-blur-sm">
          ...
          <div className="flex items-center justify-between mt-3 sm:mt-4 text-xs sm:text-sm">
            <span className="text-text-secondary">National baseline: <span className="font-semibold text-text-primary">{(BASELINE_INCLUSION_RATE * 100).toFixed(0)}%</span></span>
            <span className={`font-medium ${((prediction - BASELINE_INCLUSION_RATE) >= 0 ? 'text-brand-green' : 'text-brand-red')}`}>
              {((prediction - BASELINE_INCLUSION_RATE) * 100 >= 0 ? '+' : '')}{((prediction - BASELINE_INCLUSION_RATE) * 100).toFixed(1)}% vs baseline
            </span>
          </div>
        </div>
